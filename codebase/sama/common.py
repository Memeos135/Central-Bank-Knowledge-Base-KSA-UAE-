...
# Both regulators' rulebook platforms name PDFs identically apart from the
# prefix: SAMA_EN_<id>_VER<n>.pdf / CBUAE_EN_<id>_VER<n>.pdf (confirmed live
# for CBUAE on 2026-09-19 — e.g. CBUAE_EN_6185_VER1.pdf), so one pattern with
# an alternation covers both rather than forking this module per source.
STEM_RE = re.compile(r"((?:SAMA|CBUAE)_(?:EN|AR)_(\d+)_VER(\d+)(?:_(\d+))?)", re.I)
...
def stem_for(name: str) -> str:
    ...
    Arabic-titled documents carry no SAMA_*/CBUAE_* stem, so they keep their
    filename.
    """
    m = STEM_RE.search(name)
...
def wikilink(label: str) -> str:
    ...
    Both sources set config.NOTE_PREFIX, so the target is always the aliased,
    prefixed filename ("[[CBUAE — Board of Directors|Board of Directors]]") so
    a generic label shared with the other source's vault can never resolve
    into it, or vice versa — Obsidian resolves [[wikilinks]] by note name
    across the whole vault, ignoring folders, so the prefix has to be baked
    into every target.
    """
    prefix = getattr(config, "NOTE_PREFIX", "")
    target = f"{prefix}{strip_forbidden(label)}"
    if prefix or any(c in WIKILINK_FORBIDDEN for c in label):
