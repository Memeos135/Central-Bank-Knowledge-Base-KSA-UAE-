...
def new_page(browser, **kwargs):
    """Open a page whose User-Agent doesn't announce itself as headless.

    Playwright's headless Chromium sends `HeadlessChrome/<ver>` in its default
    User-Agent. Both SAMA's and CBUAE's rulebook sites are Drupal on Apache;
    CBUAE's WAF outright 403s any request containing that substring (confirmed
    live 2026-09-19 - same URL returns 200 to a plain HTTP client with an
    ordinary Chrome UA). Stripping "Headless" is the standard, minimal fix and
    changes nothing else about the browser's behaviour, so it's applied
    unconditionally rather than only for the source that's known to need it.
    """
    probe = browser.new_page()
    ua = probe.evaluate("() => navigator.userAgent").replace("HeadlessChrome", "Chrome")
    probe.close()
    ctx = browser.new_context(user_agent=ua, **kwargs)
    return ctx.new_page()
