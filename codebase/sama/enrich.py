...
def render(nodes: Dict[str, dict], links: List[dict], grounding: dict) -> Tuple[int, int]:
    ...
    prefix = config.NOTE_PREFIX   # "" for SAMA; "CBUAE — " once graph.namespace_vault() has run
    community_marker = f"{prefix}_COMMUNITY_"

    by_label: Dict[str, Path] = {}
    for p in config.VAULT_DIR.glob("*.md"):
        if not p.name.startswith(community_marker):
            by_label[p.stem] = p

    def find_note(label: str) -> Optional[Path]:
        from .common import strip_forbidden
        target = f"{prefix}{strip_forbidden(label)}"
        direct = config.VAULT_DIR / f"{target}.md"
        if direct.exists():
            return direct
        return by_label.get(target)
    ...
    for nid, node in nodes.items():
        ...
        tags = ["#graphify/enriched", config.SOURCE_TAG]
    ...
    for cid, name in names.items():
        ...
        # graphify's own `export obsidian` always names these stub files
        # "{marker}Community {cid}.md" - it has no knowledge of the real name
        # the label step later attaches to member nodes (community_name), so a
        # name-based lookup here never matches and comm_written was silently 0
        # on every run (confirmed empirically 2026-09-19: 318/318 community
        # enrichment records existed in enrichment.jsonl, 318/318 stub files
        # existed on disk, and every single one was skipped). Look up by cid,
        # which is how graphify actually named the file; keep the old
        # name-based glob as a fallback only in case a future graphify version
        # changes that convention. Other notes' "Connections to other
        # communities" section wikilinks to "{marker}Community {cid}", so the
        # file is intentionally left at that path - only its title/H1 and body
        # are updated to the real name.
        path = config.VAULT_DIR / f"{community_marker}Community {cid}.md"
        if not path.exists():
            matches = [p for p in config.VAULT_DIR.glob(f"{community_marker}*.md") if name in p.stem]
            if not matches:
                continue
            path = matches[0]
        lines = [f"# {name}", ""]
        ...
        # Must match the node-tag slug exactly (render()'s per-node tag above,
        # ~50 lines up) so a community's own stub note carries the same
        # #community/<slug> tag as its member nodes. This used a doubled
        # backslash (r"\\s+", matching a literal "\s" substring, never real
        # whitespace) until 2026-09-19, so the substitution silently never
        # fired: the stub note's tag kept its original spaces, which Obsidian
        # tags cannot contain, so the tag was truncated at the first space
        # (e.g. "#community/consumer" instead of
        # "#community/consumer-finance-&-credit-cards") and no query or
        # color group could ever match the stub note the way it matches its
        # own members.
        slug = re.sub(r"\s+", "-", str(name).lower())
        lines += ["", f"#community/{slug} {config.SOURCE_TAG}"]
        path.write_text("\n".join(lines) + "\n", encoding="utf-8")
        comm_written += 1
...
def routing_index(nodes: Dict[str, dict], links: List[dict]) -> Path:
    ...
    out = config.VAULT_DIR / f"{config.NOTE_PREFIX}_INDEX_Routing.md"
    out.write_text("\n".join(lines) + "\n", encoding="utf-8")
