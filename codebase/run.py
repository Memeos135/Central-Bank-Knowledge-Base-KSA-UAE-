#!/usr/bin/env python3
"""Regulatory Knowledge Base — first-run pipeline (SAMA / CBUAE).
...
Two regulators, selected with --source (default: sama). Each source has its
own scanner/corpus/graph/report directories end to end, so a CBUAE run can
never see or merge with SAMA's graph — the only thing they share is the
Obsidian vault root, where CBUAE writes into its own CBUAE/ subfolder with
every note prefixed "CBUAE — " to stay collision-safe under Obsidian's
vault-wide wikilink resolution.

    python run.py --all                       # SAMA (default), unchanged from before
    python run.py --source cbuae --all
    python run.py --source cbuae --stage crawl --manifest-only
...
# run.py now lives at <repo root>/codebase/run.py (2026-09 reorg — see
# sama/config.py's module docstring for the full directory layout), so the
# repo root — where .env lives, alongside README.md and docs/ — is one level
# up from this file, not this file's own directory.
...
def _preparse_source() -> str:
    """Pull --source off argv before sama.config is imported.

    sama.config picks its whole path/constant layout (SAMA vs CBUAE) from the
    KB_SOURCE env var at import time, so this has to happen before the `from
    sama import ...` line below — argparse itself runs far too late for that.
    Accepts `--source X` or `--source=X`; unknown values are left for the real
    argparse to reject with a proper error message.
    """
    argv = sys.argv[1:]
    ...
    return os.environ.get("KB_SOURCE", "sama").strip().lower()


os.environ["KB_SOURCE"] = _preparse_source()

from sama import audit, config, convert, crawl, enrich, graph  # noqa: E402
from sama.common import utf8_console  # noqa: E402

if config.SOURCE == "cbuae":
    from sama import crawl_cbuae  # noqa: E402
...
def main() -> int:
    ...
    ap.add_argument("--source", choices=("sama", "cbuae"), default=config.SOURCE,
                    help="which regulator's pipeline to run (also settable via KB_SOURCE; "
                         "must be given before other args are evaluated — see _preparse_source)")
    ...
    rc = 0
    for stage in stages:
        print(f"\n{'=' * 70}\n== {stage.upper()} [{config.SOURCE}]\n{'=' * 70}")
        st = time.time()
        if stage == "crawl":
            crawler = crawl_cbuae if config.SOURCE == "cbuae" else crawl
            code = crawler.run(max_pages=args.max_pages, headless=not args.headed,
