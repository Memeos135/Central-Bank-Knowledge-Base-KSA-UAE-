...
from sama import common, config
...
class TestWikilinks:
    # Both sources set config.NOTE_PREFIX (e.g. "SAMA — "), so every target is
    # the aliased, prefixed filename regardless of whether the label itself
    # contains filesystem-forbidden characters — see common.wikilink()'s
    # docstring for why the prefix must always be baked into the target.
    def test_plain_label_gets_prefixed_alias_form(self):
        prefix = config.NOTE_PREFIX
        assert common.wikilink("Banking Control Law") == (
            f"[[{prefix}Banking Control Law|Banking Control Law]]"
        )

    def test_slash_label_uses_the_alias_form(self):
        # A raw [[AML/CTF Guide]] would not resolve and would spawn a phantom node.
        prefix = config.NOTE_PREFIX
        assert common.wikilink("AML/CTF Guide") == (
            f"[[{prefix}AMLCTF Guide|AML/CTF Guide]]"
        )

    def test_colon_label_uses_the_alias_form(self):
        prefix = config.NOTE_PREFIX
        assert common.wikilink("Circular: 123") == (
            f"[[{prefix}Circular 123|Circular: 123]]"
        )
