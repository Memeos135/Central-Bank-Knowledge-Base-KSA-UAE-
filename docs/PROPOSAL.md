...
> This proposal predates the CBUAE addition and the September 2026 path reorg; its
> paths are SAMA's and reflect the original layout intent, now realised as
> `corpus/sama/`, `graph-build/sama/`, etc. under the repo root, with source moved into
> `codebase/`. See `README.md` for the current, authoritative layout.
...
navigation aids. Where they disagree with `corpus/sama/markdown/`, the corpus is right.
...
| 2 | Corpus | `corpus/sama/markdown/` — searchable text, logical-order Arabic, per-document quality grade |
| 3 | Knowledge graph | `graph-build/sama/graph.json` — deduplicated concepts, typed and confidence-tagged relationships |
...
- `corpus/sama/markdown/SAMA_EN_5565_VER1.md`
...
The **Extractor** then quotes from `corpus/sama/markdown/` directly. This is the
